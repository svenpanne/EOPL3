...
(define grammar
  '((program (expression) a-program)
    ...
    (expression ("letproc" identifier "(" (separated-list identifier ",") ")"
                           "=" expression "in" expression) letproc-exp)
    ...
    (cases proc proc1
      (procedure (vars body saved-env)
        (if (eqv? (length vars) (length vals))
            (value-of body (foldl-2 extend-env saved-env vars vals))
